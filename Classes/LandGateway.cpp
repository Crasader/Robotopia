...
{
	if( m_NextFloorNum != GET_STAGE_MANAGER()->getCurStageNum() )
	{
		m_isOpen = false;
		m_IsActive = true;
		this->removeChild( m_MainSprite );
...
cocos2d::Point LandGateway::adjustSettingPosition( cocos2d::Vec2 Idx , Direction dir )
{
	...
	{
		case DIR_RIGHT:
			Idx.y += MODULE_BASE_HEIGHT/2 + 1;
			Idx.x += 2.5f;
			break;
		case DIR_UP:
			Idx.x += MODULE_BASE_WIDTH/2 - 1;
			Idx.y += 3.5f;
			break;
		case DIR_LEFT:
			Idx.x += ( MODULE_BASE_WIDTH - 2.5f );
			Idx.y += MODULE_BASE_HEIGHT/2 + 1;
			break;
		case DIR_DOWN:
			Idx.x += MODULE_BASE_WIDTH/2 - 1;
			Idx.y += ( MODULE_BASE_HEIGHT - 3.5f );
			break;
		default:
			break;
	}
